...
| 함수 선언문 | `Func add(a, b) { return a + b; }` | ✅ | ✅ | ✅ (파라미터 이름 중복 검사, 함수 이름은 선언 즉시 정의되어 재귀 호출 허용) | 🚧 (스텁, 실행 시 예외 발생) |
| return문 | `return 1;`, `return;` | ✅ | ✅ | ✅ (함수 외부에서 사용 시 오류) | 🚧 (스텁, 실행 시 예외 발생) |
...
| 함수 호출 | `add(1, 2)` | ✅ | ✅ (연쇄 호출 `f()()` 포함) | ✅ (호출 대상과 인자 표현식 순회) | 🚧 (스텁, 평가 시 예외 발생) |
