...
함수(Chapter 2)·클래스(Chapter 3)·정적 배열·파일 모드·디버그 모드(Chapter 7)·실행 전 최적화(Chapter 5,
정적 바인딩·상수 폴딩)·import 요구사항이 모두 구현되어 단위 테스트로 검증되었습니다.
...
- **ExecutorUnit** (`ExecutorUnit/`)
  ...
  - `ConstantFolder` — `interpret()` 실행 직전에 AST를 한 번 훑어, 리터럴로만 이뤄진 연산식
    ...
    돌려줍니다.
  - `Resolver` — `interpret()` 실행 직전에 AST를 한 번 훑어, 변수/`this`/`super` 참조마다 몇 단계
    바깥 스코프에서 선언됐는지("거리")를 미리 계산해두는 전용 Visitor입니다(정적 바인딩). Checker와
    동일한 스코프 스택 방식으로 실제 `Environment` 중첩(블록/함수 호출/`for`/클래스 메서드의
    `this`·`super`)을 그대로 따라가며, `interpret()` 호출(=REPL 한 줄)마다 스코프 스택을 빈 채로
    시작합니다. 그 호출 안에서 찾지 못한 참조는 전역 변수로 간주해 `globals`로 곧장 접근합니다 -
    현재 블록 체인을 따라가면, 참조가 선언된 시점 이후 같은 이름이 다시 선언됐을 때 그 늦게 생긴
    지역 변수를 잘못 붙잡아 정적 스코프가 아닌 동적 스코프처럼 동작하는 문제가 있기 때문입니다.
    `Interpreter::evaluate`는 여기서 계산해둔 거리가 있으면 `Environment::getAt`/`assignAt`으로
    곧장 접근하고, 없으면 `globals`에서(그마저 없으면, Resolver를 거치지 않고 `CodeFabFunction`을
    직접 호출하는 화이트박스 테스트를 위해 기존 `Environment` 체인에서) 찾습니다.
  - `Environment` — 블록 진입/종료마다 생성·소멸하는 지역 스코프 체인이며, 변수 조회는 가장 안쪽
    스코프부터 전역 스코프까지 거슬러 올라가며 탐색합니다(`get`/`assign`). Resolver가 계산해둔
    거리가 있으면 `getAt`/`assignAt`으로 해당 스코프에 곧장 접근합니다.
  ...
  - `CodeFabNamespace` — `import "a.txt" alias a;`의 `a`가 평가되는 값입니다. `CodeFabInstance`와
    같은 모양으로 이름→값을 담아두는 "가방"이며, import된 파일의 최상위 `var`/`Func`/`Class`가
    여기 채워져 `a.func()`/`a.name`으로 조회됩니다. `Interpreter::executeImportStmt`가 파일을
    읽어 자신의 `AssemblerUnit`/`Checker`로 독립적으로 검사한 뒤, `globals`/`environment`를 잠시
    새 `Environment`로 바꿔치기하고 같은 Interpreter로 `interpret()`을 재귀 호출해 그 파일만의
    스코프에서 실행합니다 - Resolver/ConstantFolder를 포함한 전체 파이프라인이 그대로 재사용되고,
    가져온 파일의 전역 변수가 그대로 import한 쪽 스코프에 새는 일도 없습니다. 이미 import 중인
    경로를 다시 import하려 하면(순환 import) 예외를 던지고, import된 파일의 AST는 그 안에서
    선언된 `Func`/`Class`의 클로저가 계속 가리키므로 Interpreter가 살아있는 동안 함께 보관됩니다.
- **CodeFabFacade**
...
├── SYSTEM_DESIGN.md              # 전체 아키텍처와 모듈별 설계(현재 구현 기준)
├── README.md
└── CodeFabIntepreter/             # Visual Studio 솔루션 디렉터리
    ...
    └── CodeFabIntepreter/         # 프로젝트 디렉터리
        ...
        ├── ExecutorUnit/          # Interpreter, LineResolver, ConstantFolder, Resolver, Environment,
        │                          # CodeFabFunction, CodeFabClass, CodeFabInstance,
        │                          # CodeFabNamespace (+ 각 Test)
...
CodeFabFunction, CodeFabClass, CodeFabInstance, CodeFabNamespace, Facade, ArgumentParser, PromptShell,
FileModeShell, DebugModeShell 등)별 단위 테스트를 갖추고 있습니다. 여기에 더해 `IntegrationTest.cpp`가
클래스 상속·배열 필드·재귀·클로저(정적 바인딩)·상수 폴딩·import를 한 프로그램 안에 함께 조합해
assemble → check → interpret 전체 파이프라인으로 돌려보는 통합 테스트를 담당합니다. Debug 구성으로
빌드/실행하면 전체 테스트가 수행됩니다(현재 380개 테스트 전부 통과).
...
- `import "경로" alias 이름;`은 파일 하나를 통째로 읽어 어셈블 → (그 파일만의) 검사 → 실행까지
  마친 뒤, 그 파일의 최상위(전역) `var`/`Func`/`Class`를 `이름`이라는 값 하나에 담습니다. 이후
  `이름.멤버` 형태로만 접근할 수 있으며, 가져온 파일의 전역 변수가 현재 스코프에 직접 섞여
  들어오지는 않습니다.
- 존재하지 않는 파일을 import하면 즉시 오류가 발생합니다.
- A가 B를, B가 다시 A를 import하는 것처럼 순환 import는 오류로 감지됩니다.
- 같은 스코프에서 같은 별칭(alias)을 두 번 import하면(변수/함수/클래스 이름과 마찬가지로) 중복
  선언 오류가 발생합니다. 서로 다른 별칭으로 같은 파일을 두 번 import하는 것은 허용됩니다.
- 반복문(`for`) 몸통 안에 곧바로 있는 `import`는 오류입니다(매 반복마다 파일을 다시 읽는 것을
  막기 위함). 다만 반복문 안에서 선언된 함수 몸통 안의 `import`는 허용됩니다 - 그 함수는 호출될
  때만 실행되고 반복마다 실행되는 것이 아니기 때문입니다.
...
- 전체 아키텍처와 모듈별 설계: [SYSTEM_DESIGN.md](SYSTEM_DESIGN.md)
