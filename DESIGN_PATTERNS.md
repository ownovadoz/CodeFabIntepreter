...
`ExecutorUnit::Interpreter`, `ExecutorUnit::LineResolver`, `ExecutorUnit::ConstantFolder`,
`ExecutorUnit::Resolver`.
...
**상수 폴딩에도 재사용**: 리터럴로만 이뤄진 연산식을 실행 전에 한 번만 계산해두는
`ExecutorUnit::ConstantFolder`도 같은 Visitor를 네 번째로 구현합니다. AST 전체를
한 번 훑어 리터럴/이미 접힌 상수로만 이뤄진 노드를 찾아내고, 실제 계산은
`Interpreter::evaluate`를 그대로 재사용해 연산 로직을 중복 구현하지 않습니다.

**정적 바인딩에도 재사용**: 변수/`this`/`super` 참조가 몇 단계 바깥 스코프에서
선언됐는지 실행 전에 한 번만 계산해두는 `ExecutorUnit::Resolver`도 같은 Visitor를
다섯 번째로 구현합니다. `CheckerUnit::Checker`와 동일한 스코프 스택 방식으로 AST를
훑되, Checker는 중복 선언 검출을 위해 전역 스코프를 인스턴스 생존 기간 내내
유지하는 반면 Resolver는 호출(=REPL 한 줄)마다 스코프 스택을 빈 채로 시작해, 그
호출 안에서 찾지 못한 참조는 거리를 남기지 않고 전역 변수로 남겨둡니다.

**import 추가가 곧 이 보장의 실증**: `ImportStmt`라는 새 노드 타입을 추가했을 때
`StmtVisitor`에 `visitImportStmt`가 하나 늘었고, 그 순간 Checker/Interpreter/
LineResolver/ConstantFolder/Resolver 다섯 구현체 전부가 컴파일 에러를 냈습니다 -
`dynamic_cast` 체인이었다면 조용히 통과했을 누락(예: ConstantFolder가 import 문을
그냥 지나치는 실수)을 컴파일러가 강제로 잡아준 사례입니다.
