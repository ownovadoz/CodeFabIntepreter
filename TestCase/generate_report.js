...
const EXE = path.join(SCRIPT_DIR, '..', 'CodeFabIntepreter', 'x64', 'Release', 'CodeFabIntepreter.exe');
...
function runFile(file) {
	const result = spawnSync(EXE, ['run', path.join(SCRIPT_DIR, file)], { encoding: 'utf8' });
	return (result.stdout || '') + (result.stderr || '');
}

function buildRegularCase(caseDef) {
	console.log(`===== ${caseDef.file} =====`);
	const sourcePath = path.join(SCRIPT_DIR, caseDef.file);
...
function buildDebugCase() {
	...
	const commandLines = fs.readFileSync(path.join(SCRIPT_DIR, DEBUG_COMMANDS), 'utf8').split(/\r?\n/).filter(Boolean);
	const commands = [];
	const annotations = [];
	...
	const result = spawnSync(EXE, ['debug', path.join(SCRIPT_DIR, DEBUG_SOURCE)], { input: stdinData, encoding: 'utf8' });
